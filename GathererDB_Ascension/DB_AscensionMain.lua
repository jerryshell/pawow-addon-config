--[[
	Portions of data included here may belong to either Ascension or Blizzard.
...
elseif not Gatherer.Config.AddCallback then
	DEFAULT_CHAT_FRAME:AddMessage("GathererDB_Ascension: Please upgrade to the latest version of Gatherer.")
	lib.isLoading = false
end

if not lib.isLoading then
	DEFAULT_CHAT_FRAME:AddMessage("GathererDB_Ascension: Not loading due to missing or old Gatherer.")
	return
end
...
local function beginImport()
	...
	for zone, zonedef in pairs(zonelut) do
		...
		for node, ntype in pairs(Gatherer.Nodes.Objects) do
			Gatherer.Storage.RemoveGather(c, z, node, "DB:Ascension")
		end
		counter = counter + 1
		...
		if zonedef then
			...
			for node, ndata in pairs(zdata) do
				for pos, coord in ipairs(ndata) do
					...
					local success = Gatherer.Api.AddGather(node, nil, nil, 'DB:Ascension', nil, nil, false, c, z, x, y)
					position = position + 1
					counter = counter + 1
					if counter > YIELD_AT then
...
function lib:PerformImport()
	if not updateFrame then
		...
		updateFrame.text:SetText("Importing Ascension database:")
...
local function setupGui(gui)
	...
	local version = GetAddOnMetadata("GathererDB_Ascension", "Version")
	gui:AddControl(id, "Subhead",    0,    "Perform import of Ascension "..version.." DB:")
	...
	button.text:SetText("Data provided courtesy of |cffce0a11http://db.ascension.gg|r")
end
Gatherer.Config.AddCallback("GathererDB_Ascension", setupGui)
...
local nodeBasics = {
	...
	[244630] = { "OPEN", "Ashenvale Tree" },
	[244634] = { "OPEN", "Azshara Tree" },
	[244631] = { "OPEN", "Darkshore Tree" },
	[244620] = { "OPEN", "Dun Morogh Tree" },
	[244628] = { "OPEN", "Durotar Palm" },
	[244618] = { "OPEN", "Duskwood Tree" },
	[244614] = { "OPEN", "Elwynn Tree" },
	[244633] = { "OPEN", "Felwood Tree" },
	[244621] = { "OPEN", "Hillsbrad Tree" },
	[244622] = { "OPEN", "Hinterland Tree" },
	[244619] = { "OPEN", "Loch Modan Tree" },
	[244629] = { "OPEN", "Mulgore Tree" },
	[244627] = { "OPEN", "Plagueland Stump" },
	[244626] = { "OPEN", "Plagueland Tree" },
	[244616] = { "OPEN", "Redridge Tree" },
	[244623] = { "OPEN", "Silverpine Tree" },
	[244636] = { "OPEN", "Stonetalon Tree" },
	[244625] = { "OPEN", "Swamp Stump" },
	[244632] = { "OPEN", "Teldrassil Tree" },
	[244624] = { "OPEN", "Tirisfal Tree" },
	[244617] = { "OPEN", "Westfall Tree" },
	[244635] = { "OPEN", "Winterspring Tree" },
}
